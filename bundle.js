...
var sum = [1, 2, 3, 4, 5].reduce(accumulator, 0);
...
var ClickCounter = (function () {
   ...
   */
		...
		key: 'blueClick',
		value: function blueClick() {
			this._actions.onNext('blue');
		}
	}, {
		key: 'redClick',
		value: function redClick() {
...
redButton.onclick = function () {
	return clickCounter.redClick();
};

var blueButton = document.getElementById('blue-button');
blueButton.onclick = function () {
	return clickCounter.blueClick();
};

// Different UI components can subscribe to the same state!

// Render the score board
clickCounter.state().subscribe(function (state) {
	var html = 'Red: ' + state.red + ' Blue: ' + state.blue;
	document.getElementById('score-board').innerHTML = html;
});

// Render the current winner
clickCounter.state().subscribe(function (state) {
	var el = document.getElementById('winning');
	if (state.blue === state.red) {
		el.innerHTML = '';
		return;
	}

	var winning = state.blue > state.red ? 'blue' : 'red';
	var html = 'Winning: ' + winning;
	el.innerHTML = html;
});
